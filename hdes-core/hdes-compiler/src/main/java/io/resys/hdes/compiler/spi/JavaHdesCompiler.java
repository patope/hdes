...
import io.resys.hdes.compiler.spi.naming.JavaCompilerContext;
  ...
  public Parser parser() {
    ...
    return new Parser() {
      ...
      public List<Resource> build() {
        final CompilerContext ctx = JavaCompilerContext.config().ast(builder.build()).build();
        final List<Resource> values = ctx.ast().getBody().values().stream()
          .map(ast -> {
            
            if (ast instanceof DecisionTableBody) {
              return DtDeclarationFactory.create().body((DecisionTableBody) ast).ctx(ctx).build();
            } else if (ast instanceof FlowBody) {
              return FlDeclarationFactory.create().body((FlowBody) ast).ctx(ctx).build();
            } else {
