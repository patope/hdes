...
public class DtImplementationVisitor2 extends DtTemplateVisitor<DtJavaSpec, TypeSpec> {
  ...
  public DtImplementationVisitor2(Namings naming) {
    super();
    this.naming = naming;
  }
  ...
  public TypeSpec visitDecisionTableBody(DecisionTableBody node) {
    ...
    return TypeSpec.classBuilder(naming.dt().impl(node))
        ...
        .addAnnotation(AnnotationSpec.builder(javax.annotation.processing.Generated.class)
            .addMember("value", "$S", DtImplementationVisitor2.class.getCanonicalName())
            .build())
  ...
  public DtFormulaSpec visitFormula(Headers node) {
    ...
    ClassName outputTypeName = naming.dt().outputValueMono(body);
    ...
    for(TypeDefNode typeDef : node.getValues()) {
      ...
      if(typeDef.getDirection() == DirectionType.IN) {
        ...
        input.add(codeBlock);
        input.addStatement("src = $T.builder().from(src).$L($L).build()", JavaSpecUtil.immutable(inputTypeName), scalarDef.getName(), scalarDef.getName());
      } else {
        if(output == null) {
        ...
        output.addStatement("src = $T.builder().from(src).$L($L).build()", JavaSpecUtil.immutable(outputTypeName), scalarDef.getName(), scalarDef.getName());
      } 
    }
    ...
    if(input != null) {
      inputs.add(MethodSpec
          ...
          .addParameter(inputTypeName, "src")
    ...
    if(output != null) {
      outputs.add(MethodSpec
          .methodBuilder(APPLY_OUTPUT_FORMULA)
          .addModifiers(Modifier.PUBLIC)
          .addCode(output.addStatement("return src").build())
          .addParameter(outputTypeName, "src")
