...
public class ListTypeConverter {
  private final List<EnCodeSpec> values = new ArrayList<>();
  ...
  public ListTypeConverter value(EnCodeSpec value) {
  ...
  public ListTypeConverter value(CodeBlock value1, ScalarType type) {
    this.values.add(ImmutableEnCodeSpec.builder().value(value1).type(type).build());
    types.add(type);
    return this;
  }
  ...
  public EnListConvertionSpec build() {
    ...
    if(types.size() == 2 && types.contains(ScalarType.DECIMAL) && types.contains(ScalarType.INTEGER) ) {
      ...
      for(EnCodeSpec spec : this.values) {
